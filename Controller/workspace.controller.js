...
class WorkspaceController {
 ...
 async addMemberRequest(request, response) {
    ...
    async getById(request, response) {
        try {
            ...
            return response.json({
                status: 200,
                ok: true,
                message: "Espacio de trabajo obtenido correctamente",
                data: {
                    workspace,
                    member
                }
    ...
    async getChannels (request, response){
        Channels.findById({fk_id_workspace: request.workspace._id})
        .then((channels)=>{
            return response.json({
                status: 200,
                ok: true,
    ...
    async removeMember(request, response){
        try {
            ...
            const workspace_selected = await workspaceRepository.getById(workspace_id)
            await workspaceRepository.removeMember(workspace_selected._id, member_id)
            return response.json({
                status: 200,
                ok: true,
